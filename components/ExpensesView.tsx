...
export const ExpensesView: React.FC<ExpensesViewProps> = ({ expenses, members, currencies, onAdd, onUpdate, onDelete, onShowToast }) => {
  ...
  // Calculate totals per currency for summary
            ...
            <div className={`${mobileTab === 'input' ? 'block' : 'hidden'} lg:block bg-white p-5 rounded-[2rem] shadow-hard-sm border-2 border-beige-dark mb-10 lg:mb-0`}>
                   ...
                   <input 
                       ...
                       onChange={e => {
                           ...
                           if(val) {
                               ...
                               setForm({...form, date: d, time: t});
                           }
                       }} 
                       className="w-full bg-transparent text-cocoa outline-none font-bold text-sm" 
               ...
               <div className="grid grid-cols-3 gap-2 mb-3">
                   <div className="col-span-3 mb-1">
                       <label className="text-[10px] text-gray-400 block mb-1 font-bold">幣別 (自選)</label>
                       <select 
                           value={form.currency} 
                           onChange={(e) => setForm({...form, currency: e.target.value})} 
                           className="w-full bg-beige/50 text-cocoa rounded-xl px-3 py-2 font-bold outline-none border-2 border-beige-dark"
                       >
                           <option value="TWD">TWD (台幣)</option>
                           {currencies.map(c => (
                               <option key={c.code} value={c.code}>{c.code} (匯率 {c.rate})</option>
                           ))}
                       </select>
                   </div>
                   <div className="col-span-2">
                       ...
                       <input 
                           ...
                           className="w-full bg-beige/50 text-cocoa rounded-xl px-3 py-2 text-xl font-black outline-none focus:border-sage border-2 border-beige-dark" 
                           placeholder="0"
                       />
                   </div>
               ...
               <div className="mb-3">
                    ...
                    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                        {paymentMethods.map(method => (
                            <button key={method} onClick={() => setForm({...form, paymentMethod: method})} className={`px-3 py-1.5 rounded-lg text-xs font-bold border-2 transition-colors whitespace-nowrap ${form.paymentMethod === method ? 'bg-yellow-50 text-yellow-600 border-yellow-200' : 'bg-white text-gray-400 border-beige-dark'}`}>{method}</button>
                        ))}
                    </div>
               </div>
               ...
               <div className="mb-3">
                   ...
                   <div className="flex gap-2 mb-2">
                       <input type="text" value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="flex-1 min-w-0 w-full bg-beige/50 text-cocoa rounded-xl px-3 py-2 outline-none focus:border-sage border-2 border-beige-dark font-bold" placeholder="例如：午餐"/>
                   </div>
               </div>
               ...
               <div className="mb-4">
                   ...
                   <div className="flex gap-2 mb-3 overflow-x-auto no-scrollbar pb-1">
                       {members.map(m => (
                           <button key={m.id} onClick={() => setForm({...form, payer: m.name})} className={`px-3 py-1.5 rounded-full text-xs border-2 whitespace-nowrap transition-all flex items-center gap-1.5 active:scale-95 ${form.payer === m.name ? 'bg-sage text-white border-sage font-black shadow-sm' : 'border-beige-dark text-gray-400 bg-white font-bold'}`}>
                               {m.name}
                           </button>
                       ))}
                   </div>
                   
                   <label className="text-[10px] text-gray-400 block mb-2 font-bold">分攤成員 (Split) - 可複選</label>
                   <div className="flex gap-2 flex-wrap">
                       {members.map(m => {
                           ...
                           return (
                               <button 
                                   key={m.id} 
                                   onClick={() => toggleInvolvedMember(m.name)} 
                                   className={`px-3 py-1.5 rounded-lg text-xs border-2 transition-all flex items-center gap-1.5 active:scale-95 ${isSelected ? 'bg-blue-500 text-white border-blue-500 font-bold' : 'bg-white text-gray-400 border-beige-dark font-bold'}`}
                               >
                                   {isSelected && <Check size={12} strokeWidth={3}/>} {m.name}
                               </button>
                           );
                       })}
                   </div>
               ...
               <div className="flex gap-3">
                   <button onClick={handleBack} className="px-4 py-3.5 rounded-2xl bg-white text-gray-400 font-black border-2 border-beige-dark hover:bg-gray-50 flex items-center justify-center gap-2">
                       <ArrowLeft size={20}/> 返回
                   </button>
                   <button onClick={handleAdd} className="flex-1 bg-sage text-white py-3.5 rounded-2xl font-black shadow-hard-sage active:translate-y-1 active:shadow-none transition-all hover:bg-sage-dark text-lg tracking-wide disabled:opacity-50 disabled:cursor-not-allowed border-2 border-sage-dark flex items-center justify-center gap-2">
                       <Check size={20}/> 確認記帳
                   </button>
               </div>
            </div>
            ...
            <div className={`${mobileTab === 'list' ? 'block' : 'hidden'} lg:block bg-sage text-white p-6 rounded-[2rem] shadow-hard-sm relative overflow-hidden border-2 border-sage-dark`}>
               ...
               <div className="mt-4 flex gap-4 text-xs font-bold bg-white/20 p-2 rounded-xl inline-flex backdrop-blur-sm">
                   {Object.entries(totalsByCurrency).map(([curr, amount]) => {
                        if (curr === 'TWD' || amount === 0) return null;
                        return (
                            <div key={curr}>
                                <p className="opacity-70 text-[10px]">{curr} 支出</p>
         ...
         <div className={`${mobileTab === 'list' ? 'block' : 'hidden'} lg:block lg:col-span-7 lg:mt-0 pb-32`}>
            ...
            <div className="space-y-6 pb-20">
                        ...
                        <div key={date}>
                            ...
                            <div className="space-y-3">
                                {dailyExpenses.map(exp => (
                                    <div key={exp.id} onClick={() => openDetail(exp)} className="bg-white p-4 rounded-[1.5rem] shadow-hard-sm flex items-center gap-4 active:scale-[0.99] transition-transform cursor-pointer border-2 border-beige-dark hover:border-sage">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-start mb-0.5">
                                                ...
                                                <span className="font-black text-cocoa font-mono text-lg ml-2 whitespace-nowrap">
                                                    {exp.currency === 'KRW' ? '₩' : (exp.currency === 'USD' ? '$' : 'NT$')} {exp.amount.toLocaleString()}
                                                </span>
                                            </div>
                ...
                {filteredExpenses.length === 0 && (
                    <div className="text-center py-12 bg-white rounded-[2rem] border-2 border-dashed border-beige-dark text-gray-300">
                        <Receipt size={40} className="mx-auto mb-2" />
                        <p className="font-bold">沒有支出紀錄</p>
                    </div>
                )}
            </div>
         </div>
            ...
            <div className="bg-beige w-full max-w-sm rounded-[2rem] p-6 shadow-2xl border-4 border-beige-dark max-h-[90vh] overflow-y-auto custom-scroll">
                ...
                <div className="space-y-3">
                    ...
                    <div className="bg-white p-3 rounded-xl border-2 border-beige-dark shadow-sm">
                        ...
                        <input 
                            type="datetime-local" 
                            value={`${editingExpense.date}T${editingExpense.time}`} 
                            onChange={e => {
                                const val = e.target.value;
                                if(val) {
                                    const [d, t] = val.split('T');
                                    setEditingExpense({...editingExpense, date: d, time: t});
                                }
                            }}
                            className="w-full bg-transparent text-cocoa outline-none font-bold text-sm" 
                            style={{ colorScheme: 'light' }}
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        <div className="col-span-3 mb-1">
                             ...
                             <div className="flex gap-2 overflow-x-auto pb-1">
                                {currencies.map(curr => (
                                    <button key={curr.code} onClick={() => setEditingExpense({...editingExpense, currency: curr.code})} className={`flex-1 py-2 px-3 rounded-xl text-xs font-black border-2 whitespace-nowrap ${editingExpense.currency === curr.code ? 'bg-sage text-white border-sage' : 'bg-white text-gray-400 border-beige-dark'}`}>{curr.code}</button>
                                ))}
                            </div>
                        </div>
                        <div className="col-span-2">
                            ...
                            <input 
                                ...
                                className="w-full bg-white text-cocoa rounded-xl px-3 py-2 font-black text-lg outline-none border-2 border-beige-dark" 
                            />
                        </div>
                        <div className="col-span-1">
                            <label className="text-[10px] text-gray-400 block mb-1 font-bold">約 TWD</label>
                    ...
                    <div>
                        ...
                        <div className="flex gap-2">
                            <input type="text" value={editingExpense.title} onChange={e => setEditingExpense({...editingExpense, title: e.target.value})} className="flex-1 bg-white text-cocoa rounded-xl px-3 py-2 font-bold outline-none border-2 border-beige-dark" />
                        </div>
                    </div>

                    {/* Location */}
                    <div>
                        <label className="text-[10px] text-gray-400 block mb-1 font-bold">地點</label>
                        <input type="text" value={editingExpense.location} onChange={e => setEditingExpense({...editingExpense, location: e.target.value})} className="w-full bg-white text-cocoa rounded-xl px-3 py-2 font-bold outline-none border-2 border-beige-dark" />
                    </div>

                    {/* Payer */}
                    <div>
                        <label className="text-[10px] text-gray-400 block mb-1 font-bold">付款人</label>
                        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                            {members.map(m => (
                                <button key={m.id} onClick={() => setEditingExpense({...editingExpense, payer: m.name})} className={`px-3 py-1.5 rounded-full text-xs border-2 whitespace-nowrap font-bold ${editingExpense.payer === m.name ? 'bg-sage text-white border-sage' : 'bg-white text-gray-400 border-beige-dark'}`}>{m.name}</button>
                            ))}
                        </div>
                    </div>
                    ...
                    {/* Split Members */}
                    <div>
                        <label className="text-[10px] text-gray-400 block mb-1 font-bold">分攤成員</label>
                        <div className="flex gap-2 flex-wrap">
                           {members.map(m => {
                               const isSelected = editingExpense.involvedMembers?.includes(m.name);
                               return (
                                   <button 
                                       key={m.id} 
                                       onClick={() => toggleEditInvolvedMember(m.name)} 
                                       className={`px-3 py-1.5 rounded-lg text-xs border-2 transition-all flex items-center gap-1.5 ${isSelected ? 'bg-blue-500 text-white border-blue-500 font-bold' : 'bg-white text-gray-400 border-beige-dark font-bold'}`}
                                   >
                                       {isSelected && <Check size={12} strokeWidth={3}/>} {m.name}
                                   </button>
                               );
                ...
                <div className="flex gap-3 mt-6">
                     <button onClick={handleDelete} className="px-4 py-3 rounded-2xl bg-red-50 text-red-400 font-black border-2 border-red-100"><Trash2 size={20}/></button>
                     <button onClick={() => setShowEditModal(false)} className="flex-1 py-3 rounded-2xl bg-white text-gray-400 font-black border-2 border-beige-dark">取消</button>
                     <button onClick={handleUpdate} className="flex-1 py-3 rounded-2xl bg-sage text-white font-black shadow-hard-sage active:translate-y-1 active:shadow-none transition-all border-2 border-sage-dark">保存</button>
                </div>
            </div>
        </div>
      )}
          ...
          <div className="fixed inset-0 bg-cocoa/50 z-[100] flex items-center justify-center px-4 backdrop-blur-sm" onClick={() => setShowDetailModal(false)}>
              <div className="bg-beige w-full max-w-sm rounded-[2.5rem] p-6 shadow-2xl border-4 border-beige-dark relative overflow-hidden" onClick={e => e.stopPropagation()}>
                  <div className="text-center mb-6">
                      <div className="inline-block bg-white p-3 rounded-2xl shadow-sm border-2 border-beige-dark mb-3">
                          {selectedExpense.paymentMethod === '現金' ? <Coins size={32} className="text-orange-400"/> : <CreditCard size={32} className="text-blue-400"/>}
                      </div>
                  ...
                  <div className="space-y-4">
                      <div className="bg-white p-4 rounded-2xl border-2 border-beige-dark flex justify-between items-center shadow-sm">
                          <span className="text-xs font-bold text-gray-400 uppercase">Amount</span>
                          <div className="text-right">
                              <div className="text-2xl font-black text-cocoa font-mono">{selectedExpense.currency === 'KRW' ? '₩' : (selectedExpense.currency === 'USD' ? '$' : 'NT$')} {selectedExpense.amount.toLocaleString()}</div>
                              <div className="text-[10px] font-bold text-gray-400 font-mono">≈ NT$ {calculateTWD(selectedExpense.amount, selectedExpense.currency).toLocaleString()}</div>
                          </div>
                      </div>

                      <div className="grid grid-cols-2 gap-3">
                          <div className="bg-white p-3 rounded-2xl border-2 border-beige-dark">
                              <span className="text-[10px] font-bold text-gray-400 block uppercase mb-1">Payer</span>
                              <div className="font-black text-cocoa flex items-center gap-2">
                                  <User size={14} className="text-sage"/> {selectedExpense.payer}
                              </div>
                          </div>
                          <div className="bg-white p-3 rounded-2xl border-2 border-beige-dark">
                              <span className="text-[10px] font-bold text-gray-400 block uppercase mb-1">Method</span>
                              <div className="font-black text-cocoa flex items-center gap-2">
                                  <Wallet size={14} className="text-orange-400"/> {selectedExpense.paymentMethod}
                              </div>
                          </div>
                      </div>

                      {(selectedExpense.involvedMembers && selectedExpense.involvedMembers.length > 0) && (
                          <div className="bg-white p-3 rounded-2xl border-2 border-beige-dark">
                              <span className="text-[10px] font-bold text-gray-400 block uppercase mb-1">Shared By (分攤)</span>
                              <div className="flex flex-wrap gap-1">
                                  {selectedExpense.involvedMembers.map((m, i) => (
                                      <span key={i} className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded text-xs font-black">{m}</span>
                                  ))}
                              </div>
                          </div>
                      )}
                      ...
                      {selectedExpense.location && (
                          <div className="bg-white p-3 rounded-2xl border-2 border-beige-dark flex items-center gap-3">
                              <MapPin size={16} className="text-sage"/>
                              <span className="text-sm font-bold text-cocoa">{selectedExpense.location}</span>
                          </div>
                      )}
                  </div>

                  <div className="mt-6 pt-4 border-t-2 border-beige-dark border-dashed flex justify-between">
                      <button onClick={() => openEdit(selectedExpense)} className="text-gray-400 font-bold text-sm flex items-center gap-2 hover:text-cocoa transition-colors">
                          <PenTool size={14}/> 編輯
                      </button>
