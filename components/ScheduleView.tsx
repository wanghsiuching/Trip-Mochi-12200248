            ...
            <div className="bg-[#FDFDF5] w-full max-w-sm rounded-[2rem] p-6 shadow-2xl animate-fade-in-up border-4 border-[#E0E5D5] relative max-h-[85vh] overflow-y-auto custom-scroll">
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                        <DateTimePickerField
                            label="行程日期與時間"
                            value={modalEvent.date && modalEvent.time ? `${modalEvent.date}T${modalEvent.time}` : modalEvent.time ? `${selectedDate.date}T${modalEvent.time}` : ''}
                            onChange={val => {
                                const [d, t] = val.split('T');
                                setModalEvent({
                                    ...modalEvent,
                                    date: d || modalEvent.date || selectedDate.date,
                                    time: t || modalEvent.time || '09:00'
                                });
                            }}
                            themeColor="sage"
                            icon={Clock}
