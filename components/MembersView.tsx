        ...
        {selectedMemberId && selectedMemberData && (
            <div className="fixed inset-0 bg-cocoa/50 z-[150] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setSelectedMemberId(null)}>
                <div className="bg-beige w-full max-w-md rounded-[2.5rem] p-6 shadow-2xl border-4 border-beige-dark animate-scale-in max-h-[85vh] overflow-y-auto custom-scroll relative" onClick={e => e.stopPropagation()}>
                    <button onClick={() => setSelectedMemberId(null)} className="absolute top-4 right-4 p-2 bg-white rounded-full text-gray-400 border border-beige-dark shadow-sm"><X size={16}/></button>
        ...
        {showPasswordModal && (
            <div className="fixed inset-0 bg-cocoa/50 z-[200] flex items-center justify-center px-4 backdrop-blur-sm">
                <div className={`bg-beige w-full max-w-xs rounded-[2rem] p-6 shadow-2xl text-center border-4 border-beige-dark ${passwordShake ? 'animate-shake' : ''}`}>
                    ...
                    <input type="password" value={passwordInput} onChange={e => setPasswordInput(e.target.value)} placeholder="0000" className="w-full text-center bg-white border-2 border-beige-dark p-3 rounded-xl outline-none text-lg font-bold mb-4"/>
                    <div className="flex gap-2"><button onClick={() => setShowPasswordModal(false)} className="flex-1 py-2 text-gray-400 font-bold">取消</button><button onClick={() => {if(passwordInput==='0000'){setShowPasswordModal(false); if(pendingAction?.type==='add')setShowMemberModal(true); else if(pendingAction?.type==='delete')onDelete(pendingAction.payload);}else{setPasswordShake(true);setTimeout(()=>setPasswordShake(false),500);setPasswordInput('');}}} className="flex-1 py-2 bg-sage text-white rounded-xl font-black">確認</button></div>
                </div>
            </div>
        )}
        ...
        {showMemberModal && (
            <div className="fixed inset-0 bg-cocoa/50 z-[100] flex items-center justify-center px-4 backdrop-blur-sm">
                <div className="bg-beige w-full max-w-sm rounded-[2rem] p-6 shadow-2xl border-4 border-beige-dark">
                    <h3 className="font-black text-lg mb-6 text-center text-cocoa">新增行程成員</h3>
                    <div className="space-y-4">
                        <div><label className="text-xs text-gray-400 font-bold ml-1">名稱</label><input value={form.name} onChange={e => setForm({...form, name: e.target.value})} type="text" className="w-full bg-white text-cocoa p-3 rounded-xl outline-none border-2 border-beige-dark font-black text-center text-lg"/></div>
                    </div>
                    <div className="flex gap-3 mt-8"><button onClick={() => setShowMemberModal(false)} className="flex-1 py-3 rounded-2xl bg-white text-gray-400 font-black border-2 border-beige-dark">取消</button><button onClick={()=>{if(!form.name)return; onAdd(form.name, null); setShowMemberModal(false);}} className="flex-1 py-3 rounded-2xl bg-sage text-white font-black shadow-hard-sage border-2 border-sage-dark">儲存</button></div>
                </div>
            </div>
        )}
    </div>
