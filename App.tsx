...
import { scheduleItemToTransport } from './components/transport/adapters';
...
import { getCachedTrip, getCachedTripAsync, setCachedTrip, clearCachedTrip } from './utils/tripCache';
import { 
  useTripData, 
  useBookingsData, 
  useExpensesData, 
...
export default function App() {
  ...
  useEffect(() => {
      ...
      // 1. Instant Synchronous Pre-render from memory/localStorage
      ...
      // 2. Hydrate asynchronously from high-capacity IndexedDB for full high-res photos
      getCachedTripAsync(currentTripId).then((idbData) => {
        if (idbData) {
          applyTripData(idbData);
          setIsTripLoading(false);
        }
      }).catch(() => {});
  ...
  const openTrip = (id: string, name: string) => {
    ...
    // 2. Hydrate full IndexedDB cache (including all high-res photos)
    getCachedTripAsync(id).then((fullData) => {
      if (fullData) {
        applyTripData(fullData);
        setIsTripLoading(false);
      }
    }).catch(() => {});
                          ...
                          >
                            ...
                            <div className="bg-[#FAF9F6] p-3.5 sm:p-4">
                                <div className="space-y-3">
                                    ...
                                    {/* Transport Details - Transport Component System */}
                                    ...
                                    {item.notes && (
                                        <div className="flex gap-2 items-start bg-yellow-50/50 p-2.5 rounded-xl border border-yellow-100/50"><StickyNote size={15} className="text-yellow-500 mt-0.5 flex-shrink-0" /><p className="text-[11pt] font-medium text-cocoa whitespace-pre-wrap leading-relaxed">{item.notes}</p></div>
                                    )}
                                    {participantNames && (
